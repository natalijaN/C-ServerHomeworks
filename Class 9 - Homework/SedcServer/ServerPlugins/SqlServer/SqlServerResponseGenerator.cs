    ...
    public class SqlServerResponseGenerator : IResponseGenerator, IDisposable
    {
        ...
        private async Task<Response> GetResponse(Request request, ILogger logger)
        {
            ...
            switch (command)
            {
                ...
                case SqlResponseCommand.TableObject:
                    {
                        var tableName = path.Skip(1).First();
                        var pathObjectId = path.Skip(2).First();
                        bool success = Int32.TryParse(pathObjectId, out int objectId);
                        responder = new TableObject(ConnectionString, tableName, objectId);
                        break;
                    }
                default:
                    responder = new Error();
        ...
        private SqlResponseCommand GetCommand(IEnumerable<string> path)
        {
            ...
            if (path.Count() == 3)
            {
                ...
                if ((path.First() == "tables") && success == true)
                {
                    return SqlResponseCommand.TableObject;
                }
            }
